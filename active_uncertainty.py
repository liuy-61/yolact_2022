...
def active_uncertainty():
    ...
    #  从数据集中选出随机10%的样本来训练90个epoch初始化模型
    init_ratio = 0.1
    init_num_epochs = 90
    ...
    train(exp_name=init_exp_name,
          ...
          num_epochs=init_num_epochs)

    #  每次增量%2, 训练80个epoch
    ratio_list = [0.12, 0.14, 0.16, 0.18, 0.2, 0.22, 0.24, 0.26]
    increase_num_epochs = 80
    ...
    for i, ratio in enumerate(ratio_list):
        ...
        if i == 0:
            ...
            last_step_random_exp_name = init_exp_name
        else:
            last_step_uncertainty_exp_name = 'train_coco_active_uncertainty_sample_{}'.format(ratio_list[i-1])
            last_step_random_exp_name = 'train_coco_active_random_sample_{}'.format(ratio_list[i-1])

        last_step_uncertainty_save_folder = os.path.join(args.save_folder, last_step_uncertainty_exp_name)
        last_step_random_save_folder = os.path.join(args.save_folder, last_step_random_exp_name)

        last_step_best_uncertainty_weights_path = os.path.join(last_step_uncertainty_save_folder, 'best_mean_ap.pth')
        last_step_best_random_weights_path = os.path.join(last_step_random_save_folder, 'best_mean_ap.pth')
        ...
        train(exp_name=uncertainty_exp_name,
              ...
              num_epochs=increase_num_epochs,
              )
...
# 主动学习的中uncertainty的实验设置 优先挑选损失值大的样本
def active_uncertainty_large_loss_first():
    # 初始化随机挑选的样本和之前实验保持一致
    uncertainty_selected_list = []
    json_path = os.path.join(tmp_json_root, cfg.dataset.name + '_{}.json'.format(0.01))
    with open (json_path, 'r', encoding='utf8') as f:
        json_info = json.load(f)
        for image in json_info['images']:
            uncertainty_selected_list.append(image['file_name'])
    init_num_epochs = 30
    init_ratio = 0.1
    init_exp_name = 'train_coco_dataset_{}_epochs_with_{}_samples'.format(init_num_epochs, init_ratio)
    # 每次增量%2, 训练5个epoch
    ...
    for i, ratio in enumerate(ratio_list):
        ...
        last_step_best_uncertainty_weights_path = os.path.join(last_step_uncertainty_save_folder,
                                                               'best_mean_ap.pth')
        last_step_best_random_weights_path = os.path.join(last_step_random_save_folder, 'best_mean_ap.pth')
        ...
        train(exp_name=uncertainty_exp_name,
              dataset=uncertainty_dataset,
              num_epochs=increase_num_epochs,
              resume=last_step_best_uncertainty_weights_path)


if __name__ == '__main__':
    active_uncertainty_large_loss_first()
