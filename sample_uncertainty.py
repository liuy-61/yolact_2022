...
def sample_uncertainty(exp_setting=None, drop_ratios=None,
                        weight_to_get_loss_path=None, resume=None, num_epochs=None):
    """

    :param exp_setting: 0 去掉损失损失小的样本呢 1 去掉损失大的样本
    :param drop_ratios: 每轮实验去掉样本的比例
    :param weight_to_get_loss_path: 获取损失值的模型权重
    :param resume: 每轮实验开始之后加载该模型权重重新训练
    ...
    if args.weight_to_get_loss_path is not None:
        weight_to_get_loss_path = args.weight_to_get_loss_path
    if weight_to_get_loss_path is None:
        print("请输入参数：weight_to_get_path")
        return
    ...
    # TODO 根据训练的模型对数据集进行损失预测
    loss_list = get_loss(weight_path=weight_to_get_loss_path)
    ...
    for ratio in drop_ratios:
        exp_name = 'train_{}_uncertainty_{}_{}'.format(cfg.dataset.name, exp_setting, ratio)
