...
"""
    为MultiBoxLoss的构造函数加上了single_data 和 BFscore 参数。
    single_data为true的话求出的loss和confidence是单独对应每个样本，若为false则是一个batch样本的均值。
    BFscore为true的话，更新模型会考虑BFscore。
    
    get_confidence
    
    single_lincomb_mask_loss :计算单个样本的损失
    
    single_lincomb_mask_confidence ：计算单个样本的置信度
    
    single_lincomb_mask_loss_with_BFscore ： 
"""
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, num_classes, pos_threshold, neg_threshold, negpos_ratio, single_data=False, BFscore=False):
    ...
    def forward_single(self, net, predictions, targets, masks, num_crowds):
        """Multibox Loss
        Args:
            predictions (tuple): A tuple containing loc preds, conf preds,
            mask preds, and prior boxes from SSD net.
        ...
        # Localization Loss (Smooth L1)
        # TODO 迭代将batch_size个变量依次进行loss计算
        losses_B = []
        if cfg.train_boxes:
            for i in range(batch_size):
                loc_pi = loc_data[i][pos_idx[i]].view(-1, 4)
                loc_ti = loc_t[i][pos_idx[i]].view(-1, 4)
                losses_B.append(F.smooth_l1_loss(loc_pi, loc_ti, reduction='sum') * cfg.bbox_alpha)
        ...
        if cfg.train_masks:
            ...
            #     else:
            #         losses['M'] = self.direct_mask_loss(pos_idx, idx_t, loc_data, mask_data, priors, masks)
            # TODO 这里调用  single_lincomb_mask_loss
            if cfg.mask_type == mask_type.lincomb:
                # ret = self.lincomb_mask_loss(pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels)
                losses_M = self.single_lincomb_mask_loss(pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels)
                # if cfg.use_maskiou:
                #     loss, maskiou_targets = ret
                # else:
                #     loss = ret
        ...
        #     else:
        for i in range(batch_size):
            conf_datai = torch.unsqueeze(conf_data[i], 0)
            conf_ti = torch.unsqueeze(conf_t[i], 0)
            posi = torch.unsqueeze(pos[i], 0)
            losses_C.append(self.ohem_conf_loss(conf_datai, conf_ti, posi, 1))
        # losses['C'] = self.ohem_conf_loss(conf_data, conf_t, pos, batch_size)
        ...
        if cfg.use_semantic_segmentation_loss:
            for i in range(batch_size):
                prediction = torch.unsqueeze(predictions['segm'][i], 0)
                mask = [masks[i]]
                lable = [labels[i]]
                losses_S.append(self.semantic_segmentation_loss(prediction, mask, lable))
            # losses['S'] = self.semantic_segmentation_loss(predictions['segm'], masks, labels)
        ...
        # 对于每个样本的损失
        for i in range(batch_size):
            losses_M[i] = losses_M[i] / total_num_pos * batch_size
            losses_B[i] = losses_B[i] / total_num_pos * batch_size
            losses_C[i] = losses_C[i] / total_num_pos * batch_size

        losses_single = []
        for i in range(batch_size):
            losses_single.append(losses_B[i] + losses_S[i] + losses_C[i] + losses_M[i])
    ...
    def single_lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, interpolation_mode='bilinear'):
        mask_h = proto_data.size(1)
        mask_w = proto_data.size(2)
        ...
        for idx in range(mask_data.size(0)):
            ...
            loss_m += torch.sum(pre_loss)
            sample_losses[idx] = torch.sum(pre_loss) * cfg.mask_alpha / mask_h / mask_w
            losses_list.append(torch.sum(pre_loss) * cfg.mask_alpha / mask_h / mask_w)
            if cfg.use_maskiou:
                if cfg.discard_mask_area > 0:
                    gt_mask_area = torch.sum(mask_t, dim=(0, 1))
                    select = gt_mask_area > cfg.discard_mask_area
    ...
    def get_confidence(self, net, predictions, targets, masks, num_crowds, threshold):
        """Multibox Loss
        Args:
            predictions (tuple): A tuple containing loc preds, conf preds,
            mask preds, and prior boxes from SSD net.
        ...
        if cfg.train_masks:
            if cfg.mask_type == mask_type.lincomb:
                batch = self.single_lincomb_mask_confidence(pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, threshold)
                confidence.extend(batch)
                return confidence

    def single_lincomb_mask_confidence(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data, inst_data, labels, threshold,
        ...
        for idx in range(mask_data.size(0)):
            ...
            if cfg.mask_proto_mask_activation == activation_func.sigmoid:
                ...
                pred_np = pred_np[pred_np > threshold]
                confidence_list.append(pred_np.mean())
        return confidence_list

    # def single_lincomb_mask_loss_with_BFscore(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, score_data,
    ...
    #             在这里计算BF_loss
    #         """
    #         bfloss_criterion = BoundaryLoss()
    #         BF_losses[idx] = bfloss_criterion(torch.clamp(pred_masks, 0 ,1 ), mask_t)
    #         self.interation += 1
    #
    #     return losses , BF_losses
