...
import json
from data.local import tmp_json_root, score_root, confidence_root
...
"""
    尝试下级联方案，在目前最优效果的上串行叠加第二有效的方案。
    目前最有效的方案是置信度阈值0.3 
    第二有有效的方案是知识自蒸馏
    那尝试下用置信度先筛选完 再用知识蒸馏的教师模型筛选 
"""
def cascade_confidence_score(num_epochs=None, resume=None):
    """

    :param num_epochs: 每轮重新训练的轮数
    :return:
    """
    exp_name = 'cascade_confidence_score'
    confidence_score_list_path = os.path.join(score_root,  'confidence_threshold_0.3_with_yolact_score.pkl')
    if args.resume is not None:
        resume = args.resume
    ...
    with open(confidence_score_list_path, 'rb') as f:
        confidence_score_list = pickle.load(f)


    #  将 confidence_list 进行排序, 筛除低置信度的2%
    origin_len = len(confidence_score_list)
    confidence_score_list.sort(key=lambda x: x['confidence'], reverse=True)
    selected_list = confidence_score_list[:math.ceil(len(confidence_score_list) * 0.8)]
    selected_list.sort(key=lambda x: x['score'], reverse=True)
    selected_list = selected_list[:math.ceil(len(selected_list) * 0.875)]
    selected_list = [x['file_name'] for x in selected_list]
...
if __name__ == '__main__':
    cascade_confidence_score(num_epochs=100)
