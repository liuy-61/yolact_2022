...
from data.local import tb_dir
from data.local import loss_root
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                writer.add_scalar('lr', cfg.lr, iteration)
                #  利用tensorboard记录损失
...
def compute_validation_map(epoch, iteration, yolact_net, dataset, writer, log: Log = None):
    with torch.no_grad():
        ...
        yolact_net.train_sample_uncertainty()
...
if __name__ == '__main__':
    # get_loss(loss_root, '100_epoch.json')

    with open(os.path.join(loss_root, '100_epoch.json'), 'rb') as f:
