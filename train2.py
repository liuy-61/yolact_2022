...
# TODO 这里的 PseudoDataParallel实际上对outputs没有做任何操作 net加上PseudoDataParallel只是为了通过hook
# TODO 将devices 手动设置成[0]
class PseudoDataParallel(nn.DataParallel):
    ...
    def scatter(self, inputs, kwargs, device_ids):
        ...
        devices = [0]
        # devices = ['cuda:' + str(x) for x in device_ids]
        splits = prepare_data(inputs[0], devices, [1])
...
def compute_confidence(net, data_loader, criterion):
    confidence_list = []
    with torch.no_grad():
        iterations = 0
        for datum in data_loader:
            images, targets, masks, num_crowds, img_names = prepare_data2(datum)
            out = net(images)

            wrapper = ScatterWrapper(targets, masks, num_crowds)
            _confidence = criterion.get_confidence(out, wrapper, wrapper.make_mask())
            zipped = zip(_confidence, img_names)
            for item in zipped:
                for item in zipped:
                    confidence_dict = {}
                    confidence_dict['confidence'] = item[0]
                    confidence_dict['image_name'] = item[1]
                    confidence_list.append(confidence_dict)
        return confidence_list


def get_confidence(net, dataset):
    confidence_list = []
    timer.disable_all()
    criterion = MultiBoxLoss(num_classes=cfg.num_classes,
                             pos_threshold=cfg.positive_iou_threshold,
    ...
    confidence_list = compute_confidence(net, data_loader, criterion)
    return confidence_list
...
def get_loss():
    ...
    loss_list = []
    with torch.no_grad():
        # TODO 这里为dataset的构造函数添加一个参数，是否会返回file_name
        dataset = COCODetection(image_path=cfg.dataset.train_images,
                                info_file=cfg.dataset.train_info,
                                transform=SSDAugmentation(MEANS),
        ...
        # TODO 这里不经过 CustomDataParallel 而是 PseudoDataParallel
