...
import argparse
import datetime
from train import get_confidence_v2
# Oof
import eval as eval_script
from data.local import tb_dir, confidence_A_03path, confidence_B_03path, confidence_A_05path, confidence_B_05path
...
if __name__ == '__main__':
    # confidence_path = confidence_A_05path
    # confidence_list = get_confidence_v2(0.5)
    # with open(confidence_path, 'wb') as f:
    #     pickle.dump(confidence_list, f)

    with open('/home/algroup/liuy/yolact/confidence/coco.pkl', 'rb') as f:
        list = pickle.load(f)

    set = set()
    for item in list:
        set.add(item['file_name'])
    debug = 0