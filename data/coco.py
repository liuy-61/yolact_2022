...
class COCODetection(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """
        #  根据return_file_name参数来返回对应的返回值
        if self.return_file_name:
            im, gt, masks, h, w, num_crowds, file_name = self.pull_item(index)
            return im, (gt, masks, num_crowds), file_name
...
def detection_collate(batch):
    ...
    """
    #  这里看batch中是否有file_name,决定是否返回file_name
    return_file_name = False
    if len(batch[0]) == 3:
    ...
    for sample in batch:
        ...
        if return_file_name:
            file_name.append(sample[2])

    if return_file_name:
        return imgs, (targets, masks, num_crowds), file_name
